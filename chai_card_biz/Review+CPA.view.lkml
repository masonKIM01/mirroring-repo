  ...
  derived_table: {
    sql: select
      ...
      select
        ...
        from (select
              b2.name, b2.id,
              case when b2.name = '인테이크' then '1'
when b2.name = '인더웨어' then '1'
when b2.name = '리틀버드' then '1'
...
when b2.name = '다노샵' then '0.5'
when b2.name = '이삭토스트' then '0.5'
when b2.name = '지니' then '0.5'
when b2.name = '카모아' then '0.5'
when b2.name = '펫프렌즈' then '0.5'
when b2.name = '한섬EQL' then '0.5'
...
when b2.name = '화해' then '5000'
            ...
            where bb.merchant_ratio > 0)b2)b2 on b2.id = bpp.brand_id      )a
             ;;
  }
