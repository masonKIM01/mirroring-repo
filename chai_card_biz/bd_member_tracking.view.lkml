view: bd_member_tracking {
  derived_table: {
    sql: select
      ...
      left join
      (select x.*,
            case when x.name in ('뮬라웨어','위메프오','쿠쿠몰','바잇미','여기어때','아모레몰','얌테이블','아워홈','KKday','크로켓','롭스','무신사 ','아몬즈','무신사','캐시비','현대식품관','설로인','네파','위메프','롯데월드','화해','보고플레이','동원몰','데일리호텔','SK스토아','한섬EQL','이삭토스트','캐치패션')
            then 'cloe'
            when x.name in ('에릭요한슨 사진전','그린카','젝시믹스','펫프렌즈','인더웨어','인테이크','어바웃펫','에이블리','디코드','아몬즈','해피머니','티몬스키시즌','술담화','다노샵','프립','브랜디','W컨셉','브랜드닭','이니스프리','모던하우스','텐바이텐')
                 then 'kevin'
            else 'null'
            end as "owner"
      from
