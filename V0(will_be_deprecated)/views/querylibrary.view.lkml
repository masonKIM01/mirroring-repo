view: querylibrary {
  derived_table: {
    sql:
     SELECT CASE
             WHEN m.NAME = '차이 신용카드' THEN 'credit_card'
           ...
           p.user_id,
           CASE
             WHEN m.NAME NOT LIKE ( '%카드%' ) THEN m.NAME
             ELSE Split_part(Split_part(p.data, 'cardMerchantName":', 2), '"', 2)
           END                                                           AS
           merchantName,
           Split_part(Split_part(p.data, 'cardMerchantId":', 2), '"', 2) AS
           merchantId,
           b2.NAME                                                       AS
           boostName,
           CASE
             WHEN bpp.type = 'standard' THEN '일반'
           ...
           ad.type,
           p.idempotency_key,
           Split_part(Split_part(p.data, 'approvalNo":', 2), '"', 2)     AS
           cardApporovalNo,
           COALESCE(CASE
                      ...
                      WHEN ad.type = 'ratio'
                           AND ad.ratio > 1 THEN ad.ratio * 0.01 * (
                                                 p.cashback_amount
                                                 + COALESCE(ch.cashback_delta, 0
                                                 ) -
                    COALESCE(up.cashback_up, 0) )
                      WHEN ad.type = 'ratio'
                           AND ad.ratio < 1 THEN ad.ratio * (
                                                 p.cashback_amount
                                                 + COALESCE(ch.cashback_delta,
                                                 0)
                                                 - COALESCE(up.cashback_up, 0)
                                                            )
                    ...
                    END, 0)                                              AS adSpend,
           p.cashback_amount
           + COALESCE(ch.cashback_delta, 0)                              AS
           cashback_amount,
           p.cashback_amount
           + COALESCE(ch.cashback_delta, 0) - COALESCE(CASE
                                                         WHEN ad.type = 'cps' THEN
                                                         ad.unit_price
                                                         WHEN ad.type = 'ratio'
                                                              AND ad.ratio > 1 THEN
                                                         ad.ratio * 0.01 * (
                                              ...
                                              WHEN ad.type = 'cpa' THEN
                                              ad.unit_price * 0.5
                                                       END, 0)           AS
           chaiCredit,
           p.checkout_amount                                             AS
           checkout_amount,
           COALESCE(CASE
                      WHEN b.payment_id IS NOT NULL THEN p.checkout_amount
                    END, 0)                                              AS
           boostCheckout_amount
    FROM   chai_card_chai_prod_public.payment p
           LEFT JOIN chai_card_chai_prod_public.merchant m
                  ON m.id = p.merchant_id
           LEFT JOIN chai_card_chai_prod_public.boost b
                  ON b.payment_id = p.id
           LEFT JOIN chai_card_chai_prod_public.boost_promotion_policy bpp
                  ON bpp.id = b.boost_promotion_id
           LEFT JOIN chai_card_chai_prod_public.brand b2
                  ON b2.id = bpp.brand_id
           LEFT JOIN (SELECT DISTINCT year,
                     ...
                     chai_card_chai_prod_public.delayed_cashback_history
                      ...
                      GROUP  BY 1,
                                ...
                                3,
                                4) ch
                  ON ch.payment_id = p.id
           LEFT JOIN (SELECT b.payment_id,
                             ad.type,
                             unit_price,
                             ratio,
                             b.year,
                      ...
                      FROM   chai_card_chai_prod_public.boost b
                             INNER JOIN
                             chai_card_chai_prod_public.boost_campaign_ad_spend
                             ad
