...
select '2021-12-01' as months, '원데이즈유' as merchant_name, '10,000원 캐시백' as title, 'CPS' as type, '0' as merchant_ratio, '6500' as contract, '0' as CPA_done union all
  ...
  select '2022-01-01' as months, '아이디어스' as merchant_name, '10,000원 캐시백' as title, 'CPS' as type, '0' as merchant_ratio, '3500' as contract, '0' as CPA_done)
      ...
      WHERE (table_redshift_payment.status ) = 'confirmed' and (table_redshift_payment.year ) in ('2021','2022') AND (table_redshift_payment.month ) IN (10,11,12,1)
      GROUP BY
          1,
          2,
          3,
  ...
  measure: sum_adspend{
    ...
    sql: case when ${table_merchant_adspend_type} = 'CPA' then ${table_merchant_adspend_contract}::numeric * ${table_redshift_boost_count_boost_id} * 0.2
              when ${table_merchant_adspend_merchant_ratio} > 0 then ${table_merchant_adspend_merchant_ratio}::decimal(3,2) * ${table_redshift_payment_total_cashback_amount}
